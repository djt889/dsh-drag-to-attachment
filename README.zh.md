...
> 一个面向 [DeepSeek Harness](https://github.com/deepseek-ai/dsh) Web UI 的 **dsh-plugin**：把本地**任意文件或文件夹**拖入（或粘贴进）页面任意位置——一个开关，两种模式：**附件模式**把**图片**以原生缩略图附件呈现、发送时保持缩略图形式；**其他文件/文件夹**识别其**真实路径**以附件方块引用（**不复制、不上传**），发送时把真实路径随消息送达模型；**路径模式**直接把真实路径插入输入框草稿。
...
- **图片**保持 DSH 原生附件体验（缩略图卡片、预览、移除），发送时**保持缩略图形式**，不转换。
...
|---|---|---|
| 任意位置拖入 / 粘贴 | ✅ | ✅ |
| 图片 | **原生缩略图附件**（发送保持缩略图形式） | 插入真实路径 |
| 任意文件（不限扩展名） | 附件方块（**引用真实路径**，hover 显示） | 插入真实路径 |
| 文件夹 | 附件方块 📁（引用文件夹路径） | 插入文件夹路径 |
| 发送 | 图片按原生缩略图发送；文件/文件夹附真实路径文本 | 路径已在草稿中，直接发送 |
| 是否复制/上传文件 | **从不**（不产生 `.drops/` 副本） | 从不 |
| 不输入文字直接发送 | ✅（不可见占位符自动激活发送按钮） | — |

> 说明：图片以**原生附件（缩略图）**发送，需要模型支持图片输入（支持图片的模型会直接在会话中显示图片；纯文本模型发送图片附件会被宿主预检拦截——此时请改用**路径模式**，或仅发送文件/文件夹）。文件/文件夹必须能被定位到**真实路径**（工作区优先，再已注册工作区、桌面/文档/下载、系统索引、受控递归搜索）；定位失败会弹出可见提示，不会静默丢弃。
