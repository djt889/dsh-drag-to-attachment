 ...
 *
 * Two exact HTTP routes on the DSH webServer:
 *
 *   POST /_dsh/drag-to-attachment/import  { name, dataBase64, workspace?, relPath? }
 *     Upload route (attachment mode): writes the decoded file into the active
 *     session workspace `.drops/` dir and returns the absolute path. ANY file
 *     type is accepted (no extension whitelist); folders are supported by
 *     uploading each member file with a `relPath` (the returned `folderPath`
 *     points at the folder root).
 *
 *   POST /_dsh/drag-to-attachment/locate { file, phase, ... }
 *     Locator route (path mode): resolves the real filesystem path of a
 *     dropped/pasted file or folder. Ported from bill9109/dsh-drag-and-drop
 *     (multi-phase: metadata → sample/full fingerprint → directory structure/
 *     content; Workspace-first, then registered workspaces, common dirs,
 *     OS index and bounded recursive search).
 */
...
export const IMPORT_ROUTE = '/_dsh/drag-to-attachment/import'
...
async function workspaceRoot() {
  ...
  // "browser-sessions" project must never be an upload root), then the
...
/**
 * Validate a folder-relative path (e.g. `myfolder/sub/a.txt`). Only plain
 * relative segments of letters/digits/space/dot/dash/underscore/Unicode are
 * allowed; `..`, absolute paths, drive letters and separators are rejected.
 */
function validRelPath(raw) {
  ...
  if (segments.length === 0 || segments.length > MAX_REL_SEGMENTS) return null
...
async function handleImport(req, res) {
  ...
  try {
    ...
    const { name: rawName, dataBase64, workspace: clientWorkspace, relPath: rawRelPath } = body
    // dataBase64 may be an empty string for 0-byte files inside folders.
    if (typeof dataBase64 !== 'string') {
      respond({ ok: false, error: { code: 'invalid-request', message: 'Missing dataBase64' } }, 400)
      return
    }
    // Trust the client-supplied active workspace only when it is an absolute
    // path; otherwise fall back to the registry scan.
    const root = typeof clientWorkspace === 'string' && isAbsolute(clientWorkspace)
      ? clientWorkspace
      : await workspaceRoot()
    const bytes = Buffer.from(dataBase64, 'base64')
    if (bytes.length > MAX_FILE_BYTES) {
      respond({ ok: false, error: { code: 'too-large', message: `File exceeds ${Math.floor(MAX_FILE_BYTES / 1024 / 1024)}MB` } }, 413)
      return
    }

    const dropRoot = join(root, DROP_DIR)
    let target, relPath
    if (rawRelPath !== undefined && rawRelPath !== null && String(rawRelPath).length > 0) {
      ...
      target = join(dropRoot, relPath)
    } else {
      const safe = safeName(rawName)
      target = join(dropRoot, `${Date.now()}-${safe}`)
    }
    const normalizedTarget = normalize(target)
    const normalizedRoot = normalize(dropRoot) + sep
    if (!normalizedTarget.startsWith(normalizedRoot)) {
      respond({ ok: false, error: { code: 'invalid-request', message: 'Path escapes drop root' } }, 400)
      return
    }

    await mkdir(dirnameOf(normalizedTarget), { recursive: true })
    await writeFile(normalizedTarget, bytes)

    let folderPath = null
    if (relPath !== null && relPath !== undefined) {
      const first = relPath.split(sep)[0]
      folderPath = join(dropRoot, first)
    }
    respond({
      ok: true,
      value: {
        ...
        folderPath,
        filename: basename(normalizedTarget),
        bytes: bytes.length,
        kind: folderPath ? 'dir-member' : 'file',
      },
    })
  } catch (error) {
...
export async function apply(ctx) {
  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => {
      const disposeImport = webCtx.webServer.register({
        kind: 'exact',
        path: IMPORT_ROUTE,
        handler: (req, res) => handleImport(req, res),
      })
      ...
      return () => { disposeImport(); disposeLocate() }
    }, 'drag-to-attachment: import + locator routes')
  })
}
