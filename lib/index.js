 ...
 *     Fallback save (used for pasted screenshots and unlocatable files):
 *     writes them into <workspace>/.drops/ so the attachment has a real
 *     path the agent can read. ≤100MB.
 *
...
export const MD5_ROUTE = '/_dsh/drag-to-attachment/md5'
...
// Built-in Everything (freeware, https://www.voidtools.com): bundled with the
// plugin so full-disk name search is millisecond-fast with zero user install.
// The host starts Everything.exe (portable) on boot and queries it through
// es.exe; its stdout is GBK on Chinese Windows, decoded with TextDecoder.
const PACKAGE_ROOT = dirname(dirname(fileURLToPath(import.meta.url)))
const EVERYTHING_DIR = join(PACKAGE_ROOT, 'vendor', 'everything')
...
/** Start the bundled Everything once if it is not already running. */
async function ensureEverything() {
  if (everythingEnsured) return
  everythingEnsured = true
  try {
    // Already running (bundled earlier or user-installed) → skip.
...
/** Resolve the es.exe to use: bundled first, then PATH. */
async function builtinEs() {
  try { await access(ES_EXE, constants.X_OK); return ES_EXE } catch (error) { /* fall through */ }
  return null
}

/** Run es.exe and decode its output (GBK on Chinese Windows). */
async function esSearch(es, args) {
  const { stdout } = await execFileAsync(es, args, {
    encoding: 'buffer', timeout: 15000, windowsHide: true, maxBuffer: 4 * 1024 * 1024,
  })
...
async function handleSave(req, res) {
  ...
  try {
    ...
    const { name: rawName, dataBase64, relPath: rawRelPath } = body
    ...
    const dir = join(root, '.drops')
    await mkdir(dir, { recursive: true })
    let target, relPath
    if (rawRelPath !== undefined && rawRelPath !== null && String(rawRelPath).length > 0) {
      relPath = validRelPath(rawRelPath)
      if (relPath === null) {
        respond({ ok: false, error: { code: 'invalid-request', message: 'Invalid relPath' } }, 400)
        return
      }
      target = join(dir, relPath)
    } else {
      // Content-addressed name: identical pasted screenshots hash to the
      // SAME file, so repeated pastes never pile up new copies — the
      // dedupe check then reports "already uploaded".
      const hash = createHash('md5').update(bytes).digest('hex')
      target = join(dir, `${hash}-${safeName(rawName)}`)
    }
    const normTarget = normalize(target)
    const normDir = normalize(dir) + sep
    if (!normTarget.startsWith(normDir)) {
      respond({ ok: false, error: { code: 'invalid-request', message: 'Path escapes save root' } }, 400)
      return
    }
    const parent = normTarget.slice(0, normTarget.lastIndexOf(sep))
    await mkdir(parent, { recursive: true })
    // Idempotent: an existing file with the same content is returned as-is
    // (top-level saves only; folder members keep overwrite semantics).
    if (relPath === null || relPath === undefined) {
      try {
        const existing = await stat(normTarget)
        if (existing.isFile()) {
          respond({ ok: true, value: { path: normTarget, folderPath: null, filename: basename(normTarget), bytes: bytes.length, duplicate: true } })
          return
        }
      } catch { /* not exists → write below */ }
    }
    await writeFile(normTarget, bytes)
    let folderPath = null
    if (relPath !== null && relPath !== undefined) {
      folderPath = join(dir, relPath.split(sep)[0])
    }
    respond({ ok: true, value: { path: normTarget, folderPath, filename: basename(normTarget), bytes: bytes.length } })
  } catch (error) {
    respond({ ok: false, error: { code: 'save-failed', message: error instanceof Error ? error.message : String(error) } }, 500)
  }
}

/** MD5 of one existing file (used by the attachment dedupe check). */
async function handleMd5(req, res) {
  const respond = (value, status = 200) => {
    res.writeHead(status, { 'Content-Type': 'application/json' })
    res.end(JSON.stringify(value))
  }
  try {
    ...
    let body
    try {
      body = JSON.parse(await readBody(req, 1 * 1024 * 1024))
    } catch (error) {
      respond({ ok: false, error: { code: 'invalid-request', message: 'bad request' } }, 400)
      return
    }
    const { path } = body
    if (typeof path !== 'string' || path.length === 0) {
      respond({ ok: false, error: { code: 'invalid-request', message: 'Missing path' } }, 400)
      return
    }
    const info = await stat(path)
    if (!info.isFile()) {
      respond({ ok: false, error: { code: 'not-found', message: 'not a file' } }, 404)
      return
    }
    ...
    respond({ ok: true, value: { md5: hash.digest('hex') } })
  } catch (error) {
    respond({ ok: false, error: { code: 'md5-failed', message: error instanceof Error ? error.message : String(error) } }, 500)
  }
}

/** Validate a folder-relative path (folder members only): plain relative
 *  segments, no separators/dots/absolutes/drive letters. Returns the native
 *  separator form, or null when invalid. */
function validRelPath(raw) {
  ...
  for (const segment of segments) {
    ...
    if (/[\\/:*?"<>|\x00-\x1f]/.test(segment)) return null
  }
...
// --- Linux: built-in Everything-style filename index (no install needed).
// The host scans user dirs + mounts in the background and answers name
// queries from memory — millisecond hits like Everything/plocate.

const MAX_LINUX_INDEX_NAMES = 2_000_000
let linuxNameIndex = null
let linuxIndexPromise = null

function linuxIndexRoots() {
  const roots = [homedir()]
...
async function walkLinuxIndex(dir, index, depth) {
  if (depth > 12 || index.size >= MAX_LINUX_INDEX_NAMES) return
  let entries
  try { entries = await readdir(dir, { withFileTypes: true }) } catch { return }
  for (const entry of entries) {
    if (index.size >= MAX_LINUX_INDEX_NAMES) return
    if (entry.isSymbolicLink()) continue
    const full = join(dir, entry.name)
    if (entry.isDirectory()) {
      // Skip virtual/special filesystems and obvious noise.
      if (entry.name === 'proc' || entry.name === 'sys' || entry.name === 'dev' ||
          entry.name === 'run' || entry.name === 'tmp' || entry.name === 'snap' ||
          entry.name === '.cache') continue
      await walkLinuxIndex(full, index, depth + 1)
...
function ensureLinuxIndex() {
  if (linuxNameIndex !== null || linuxIndexPromise !== null) return linuxIndexPromise
  linuxIndexPromise = (async () => {
    const index = new Map()
    const start = Date.now()
    try {
      for (const root of linuxIndexRoots()) {
        await walkLinuxIndex(root, index, 0)
      }
      linuxNameIndex = index
      console.log(`[drag-to-attachment] linux index ready: ${index.size} names in ${Date.now() - start}ms`)
    } catch (error) {
      console.error('[drag-to-attachment] linux index failed:', error)
      linuxNameIndex = new Map()
    }
  })()
  return linuxIndexPromise
}

async function linuxSearch(name, runtime) {
  // Built-in index first (millisecond, no install).
  ensureLinuxIndex()
  if (linuxNameIndex !== null) {
    const hits = linuxNameIndex.get(name)
    if (hits !== undefined && hits.length > 0) return hits.slice(0, 100)
  }
...
async function windowsSearch(name, runtime) {
  // Bundled Everything (es.exe) first: millisecond full-disk name search.
  const es = await builtinEs()
  if (es !== null) {
    try {
      return lines(await esSearch(es, ['-n', String(100), name]))
    } catch (error) { /* fall through */ }
  }
  for (const command of ['es.exe', 'Everything.exe']) {
    if (!await runtime.commandExists(command)) continue
    try {
      return lines(await runtime.exec(command, ['-n', String(100), name]))
    } catch { /* try next */ }
  }
...
// Generous search bounds: deep directories (WeChat files etc.) and whole
// fixed drives need more than the conservative upstream limits. The client
// shows a loading ring and waits up to 20s, so the host can afford it.
const MAX_WALK_ENTRIES = 1e5
const WALK_DEPTH = 24
...
/** Build the ordered search-root groups for one request. */
function metadataRootGroups(request) {
...
/**
 * FAST tier: shallow probes (ms) then the OS index (Everything/Spotlight/
 * built-in Linux index — ms). Returns candidates or [].
 */
async function fastCandidates(item, request) {
  const rootGroups = metadataRootGroups(request)
  ...
  return validateCandidates(item, indexedPaths)
}

/**
 * FULL tier: bounded recursion over all roots, then broad mounts. Returns
 * candidates or [].
 */
async function fullCandidates(item, request) {
  const rootGroups = metadataRootGroups(request)
  for (const roots of rootGroups) {
...
async function metadataCandidates(item, request) {
  const fast = await fastCandidates(item, request)
  if (fast.length > 0) return fast
  return fullCandidates(item, request)
}
...
async function locate(request) {
  ...
  // Progressive search tiers used by the real-time progress UI: 'tier-fast'
  // runs shallow probes + OS index (ms), 'tier-full' runs the bounded
  // recursion. (Distinct from the fingerprint phases 'sample'/'full'.)
  if (request.phase === 'tier-fast' || request.phase === 'tier-full') {
    const candidates = request.phase === 'tier-fast'
      ? await fastCandidates(request.file, request)
      : await fullCandidates(request.file, request)
...
/**
 * Model-facing tool: insert a local image into the current conversation so
 * the user sees it inline (ChatGPT-style preview). The tool returns a
 * markdown image pointing at the plugin's read-only /file proxy; the agent
 * pastes it into its reply and DSH's MarkdownText renders the image in the
 * conversation. No upload, no copy — the image stays where it is.
 */
async function registerImagePreviewTool(ctx) {
  let tools
  try { tools = ctx.get('tools') } catch (error) { tools = undefined }
  if (tools === undefined) return undefined
  let defineTool
  try {
    ({ defineTool } = await import('@deepseek-ai/dsh-tools'))
  } catch (error) {
    console.error('[drag-to-attachment] dsh-tools unavailable:', error)
    return undefined
  }
  if (typeof defineTool !== 'function') return undefined
  const tool = defineTool({
    name: 'image_preview',
    description: '把一张本地图片插入当前会话显示给用户预览（例如生图、截图、转换结果）。调用后把返回的 markdown 图片放进你的回复正文，图片就会直接显示在对话里（无需上传、不复制文件）。',
    parameters: {
      path: { type: 'string', required: true, description: '本地图片的绝对路径（png/jpg/jpeg/webp/gif/bmp，≤20MB）' },
      caption: { type: 'string', description: '可选：图片说明文字（缺省用文件名）' },
    },
    output: {
    ...
    async execute(args) {
      ...
      if (typeof path !== 'string' || path.length === 0) throw new Error('image_preview: path is required')
      const info = await stat(path)
      if (!info.isFile()) throw new Error('image_preview: not a file: ' + path)
      const ext = extname(path).toLowerCase()
      if (IMAGE_TYPES[ext] === undefined) throw new Error('image_preview: unsupported image type: ' + ext)
      if (info.size > MAX_THUMB_BYTES) throw new Error('image_preview: image too large (≤20MB)')
      const filename = basename(path)
      // MarkdownText only renders http(s) image URLs, so build an absolute
      // one against the host's own web URL (DSH_WEB_URL).
      const base = (typeof process !== 'undefined' && process.env && process.env.DSH_WEB_URL)
        ? String(process.env.DSH_WEB_URL).replace(/\/+$/, '')
        : ''
      const url = base + FILE_ROUTE + '?path=' + encodeURIComponent(path)
      const label = (typeof args.caption === 'string' && args.caption.length > 0) ? args.caption : filename
      return { markdown: '![' + label + '](' + url + ')' }
    },
    presentCall: () => ({ card: 'generic', title: '插入图片预览', kind: 'execute' }),
  })
  try {
    return tools.register(tool)
  } catch (error) {
    console.error('[drag-to-attachment] image_preview register failed:', error)
    return undefined
  }
}

export async function apply(ctx) {
  // Kick the bundled Everything into life (Windows only, best-effort) so
  // full-disk name search is instant; on Linux build the built-in index.
  if (platform() === 'win32') void ensureEverything()
  else if (platform() === 'linux') ensureLinuxIndex()
  void registerImagePreviewTool(ctx)
  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => {
      ...
      const disposeMd5 = webCtx.webServer.register({
        kind: 'exact',
        path: MD5_ROUTE,
        handler: (req, res) => handleMd5(req, res),
      })
      return () => { disposeLocate(); disposeFile(); disposeSave(); disposeMd5() }
    }, 'drag-to-attachment: locator + save + md5 + thumbnail routes')
  })
}
