...
async function metadataCandidates(item, request) {
  ...
  // Order: shallow probes (ms) → OS index via bundled Everything (ms) →
  // bounded recursion (slow, only if the fast tiers missed). The index is
  // consulted BEFORE recursion so deep files (WeChat etc.) resolve in
  // milliseconds instead of after a long directory walk.
