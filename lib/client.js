  ...
  factory: (require) => {
    ...
    function handlePathDrop(ctx, dataTransfer, toast) {
      ...
      dropped.directories.forEach(function (directory) {
        entries.push({ name: directory.name, locate: function () { return locateDroppedDirectoryTimed(directory, ctx.workspaces, currentWorkspacePath(ctx), 8000) } })
      })
      dropped.files.forEach(function (file) {
        entries.push({ name: file.name, locate: function () { return locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 8000) } })
      })
      if (entries.length === 0) return
      // One spinning chip per entry so the user can see the search working
      // (parallel resolution; chips disappear as each path lands).
      entries.forEach(function (entry) { addPathPending(entry.name) })
      var found = []
      var failures = []
      Promise.all(entries.map(function (entry) {
        return entry.locate().then(function (result) {
    ...
    function handlePathPaste(ctx, clipboardData, toast) {
      ...
      Promise.all(promises).then(function () {
        ...
        files.forEach(function (file) {
          ...
          tasks.push(locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 8000).then(function (result) {
        ...
        dirs.forEach(function (dir) {
          ...
          tasks.push(locateDroppedDirectoryTimed(dir, ctx.workspaces, currentWorkspacePath(ctx), 8000).then(function (result) {
    ...
    function renderChips() {
      ...
      fileQueue.forEach(function (item) {
        ...
        // Pending (path not resolved yet): overlay a small loading ring.
        if (!item.path) {
          var spinner = document.createElement('span')
          spinner.className = 'dta-spinner'
          spinner.setAttribute('aria-label', '定位中')
          chip.append(spinner)
        }
    ...
    function showPathPendingBar() {
      ...
      pathPending.forEach(function (item) {
        ...
        var spinner = document.createElement('span')
        spinner.className = 'dta-spinner'
        spinner.style.cssText = 'position:relative;top:auto;right:auto;width:14px;height:14px;flex:0 0 auto'
        var label = document.createElement('span')
    ...
    function addPathPending(name) { pathPending.push({ name: name }); showPathPendingBar() }
    ...
    function hasPendingPaths() {
      return fileQueue.some(function (q) { return !q.path })
    }
    function syncPlaceholder() {
      ...
      if (fileQueue.length > 0 && !hasPendingPaths() && draft.trim() === '' && !hasImages) {
    ...
    function locateAndQueue(ctx, files, dirs, toast) {
      ...
      var batch = []
      files.forEach(function (item) {
        var queued = { path: '', name: item.file.name, previewUrl: item.previewUrl || null }
        addQueued(queued)
        batch.push(resolveFilePath(item.file, queued, ctx, failures))
      })
      dirs.forEach(function (dir) {
        var queued = { path: '', name: dir.name, isDir: true }
        addQueued(queued)
        batch.push(resolveDirPath(dir, queued, ctx, failures))
      })
    ...
    function dropQueued(queued, failures, message) {
      var idx = fileQueue.indexOf(queued)
      if (idx !== -1) { fileQueue.splice(idx, 1); renderChips(); syncPlaceholder() }
      failures.push(message)
    }

    function resolveFilePath(file, queued, ctx, failures) {
      return locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 8000)
        .then(function (result) {
          if (result.status === 'found') { queued.path = result.path; renderChips(); syncPlaceholder(); return }
          if (result.status === 'choose') {
            return choosePath(file.name, result.candidates).then(function (selected) {
              if (selected !== undefined) { queued.path = selected; renderChips(); syncPlaceholder() }
              else dropQueued(queued, failures, file.name + '（未选择路径）')
            })
          }
          ...
          return saveFileToWorkspace(file).then(function (value) {
            queued.path = value.path; renderChips(); syncPlaceholder()
    ...
    function resolveDirPath(dir, queued, ctx, failures) {
      return locateDroppedDirectoryTimed(dir, ctx.workspaces, currentWorkspacePath(ctx), 8000)
        .then(function (result) {
          if (result.status === 'found') { queued.path = result.path; renderChips(); syncPlaceholder(); return }
          if (result.status === 'choose') {
            return choosePath(dir.name, result.candidates).then(function (selected) {
              if (selected !== undefined) { queued.path = selected; renderChips(); syncPlaceholder() }
              else dropQueued(queued, failures, '文件夹「' + dir.name + '」未选择路径')
            })
          }
          ...
          return saveFolderToWorkspace(dir.entry, dir.name).then(function (folderPath) {
            if (folderPath) { queued.path = folderPath; renderChips(); syncPlaceholder() }
    ...
    function patchSendSession(conversation, sessions, workspaces) {
      ...
      proto.sendSession = async function (session, text, imageIds, mode) {
        ...
        // background — wait for that resolution before sending so every
        // queued attachment has its real path (or was already dropped).
        if (pendingLocates.length > 0) await Promise.all(pendingLocates)
      ...
      try {
        ...
        if (!document.getElementById(styleId)) {
          ...
          style.textContent =
            ...
            '.dta-spinner{position:absolute;top:3px;right:3px;width:14px;height:14px;box-sizing:border-box;' +
            'border:2px solid rgba(255,255,255,.45);border-top-color:#ffffff;border-radius:50%;' +
            'animation:dta-spin .8s linear infinite;z-index:2}' +
            '@keyframes dta-spin{to{transform:rotate(360deg)}}'
