...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Locate the real path of every dropped/pasted item and queue it as an
     * attachment. Nothing is uploaded or copied: the queued path IS the
     * original filesystem path, and on send it is appended to the message
     * (the agent reads the file with vision-toolkit / document tools).
     */
    function locateAndQueue(ctx, files, dirs, toast) {
      var found = []
      var failures = []
      var chain = Promise.resolve()
      files.map(function (item) { return item.file }).forEach(function (file) {
        chain = chain.then(function () { return locateDroppedFile(file, ctx.workspaces, currentWorkspacePath(ctx)) }).then(function (result) {
          if (result.status === 'found') found.push({ path: result.path, name: file.name })
          else if (result.status === 'choose') {
            return choosePath(file.name, result.candidates).then(function (selected) {
              if (selected !== undefined) found.push({ path: selected, name: file.name })
              else failures.push(file.name)
            })
          } else if (result.status === 'error') failures.push(file.name + '（' + result.message + '）')
          else failures.push(file.name)
      ...
      chain.then(function () {
        ...
        if (failures.length > 0) {
          toast.showError('未能定位原始路径（附件仅引用、不复制文件）：' + failures.join('、'))
        }
      })
    }

    /**
     * Locate the real path of every dropped/pasted non-image file and folder,
     * then queue it as a chip. Nothing is uploaded or copied — the queued
     * path IS the original filesystem path; on send it is appended to the
     * message (images keep their native attachment form and are NOT
     * converted to paths).
     */
    function locateAndQueue(ctx, files, dirs, toast) {
      var found = []
    ...
    /**
     * Images keep the NATIVE attachment experience: real thumbnail cards
     * (preview / remove). Nothing is copied — the native draft image is a
     * browser blob; the underlying file stays where it is. Sending also
     * stays native (no path conversion).
     */
    function attachTopLevelImages(images, ctx, toast) {
      var conversation = ctx.get('conversation')
      var input = currentInput(ctx)
      if (conversation === undefined || input === undefined) {
        // No session: cannot use the native rail — fall back to locating.
        return locateAndQueue(ctx, images.map(function (file) { return { file: file, relPath: undefined } }), [], toast)
      }
      try {
        var created = conversation.createDraftImages(images)
        if (input.addImages(created.map(function (a) { return a.id }))) return
        conversation.releaseDraftImages(created)
      } catch (error) { /* unsupported image type → locate as a plain file */ }
      locateAndQueue(ctx, images.map(function (file) { return { file: file, relPath: undefined } }), [], toast)
    }

    function handleAttachmentDrop(ctx, dataTransfer, toast) {
      collectDrop(dataTransfer).then(function (dropped) {
        if (dropped.images.length > 0) attachTopLevelImages(dropped.images, ctx, toast)
        locateAndQueue(ctx, dropped.files, dropped.dirs, toast)
      }).catch(function (error) {
    ...
    function handleAttachmentPaste(ctx, clipboardData, toast) {
      ...
      Promise.all(promises).then(function () {
        if (images.length > 0) attachTopLevelImages(images, ctx, toast)
    ...
    // --- send-time: append the ORIGINAL paths (attachment mode, no copying) ---
    function patchSendSession(conversation, sessions, workspaces) {
      ...
      proto.sendSession = async function (session, text, imageIds, mode) {
        ...
        // No file/folder chips queued → keep the NATIVE send path verbatim:
        // plain text and/or draft images are sent as-is (images stay
        // thumbnails in the conversation; nothing is converted to paths).
        if (queued.length === 0) return original.call(this, session, text, imageIds, mode)

        var paths = queued.map(function (item) { return item.path })
        // Mixed batch (file chips + native images): images cannot ride the
        // native channel together with the text-only path lines, so their
        // real paths are located and appended too — nothing is copied.
          ...
          if (attachments.length === imageIds.length) {
            ...
            for (var i = 0; i < attachments.length; i++) {
              ...
              try {
                ...
                if (result.status === 'found') paths.push(result.path)
                else if (result.status === 'choose') {
                  var selected = await choosePath(file.name, result.candidates)
                  if (selected !== undefined) paths.push(selected)
                } else {
