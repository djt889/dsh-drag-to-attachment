...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // --- which normally carries copy IconActions — re-provide them, styled
    // --- exactly like the native MessageIconActions (28px round, theme hover,
    // --- hover-revealed time, 10px gap).
    ...
    function renderUserActions(text, timeMs) {
      ...
      if (timeMs !== undefined) {
        try {
          ...
          items.push(React.createElement('span', { key: 'time', className: 'dta-user-time', style: { paddingRight: '12px', fontSize: '14px', lineHeight: '24px', color: 'var(--dsw-alias-label-tertiary)', whiteSpace: 'nowrap' } },
      ...
      if (hasText) {
        ...
        items.push(React.createElement('button', { key: 'edit', type: 'button', className: 'dta-user-action', title: '编辑', 'aria-label': '编辑', onClick: function () { editUserText(text) }, style: iconBtnStyle() }, iconSvg(EDIT_ICON_PATH)))
      }
    ...
    function UserMessageView(props) {
      try {
        ...
        if (parsed === null) {
          ...
          return React.createElement('div', rowAttrs, React.createElement('div', { style: row },
      ...
      try {
        ...
        if (!document.getElementById(styleId)) {
          ...
          style.textContent =
            ...
            '.dta-user-action{transition:background-color .12s ease,color .12s ease}' +
            '.dta-user-action:hover{background:var(--dsw-alias-interactive-bg-hover);color:var(--dsw-alias-label-secondary)}' +
            '@media(hover:hover){[data-time-hover-root] .dta-user-time{opacity:0;transition:opacity 80ms ease}' +
            '[data-time-hover-root]:hover .dta-user-time,[data-time-hover-root]:focus-within .dta-user-time{opacity:1}}'
