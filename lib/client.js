...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // --- which normally carries copy/edit IconActions — re-provide them.

    function userActionStyle() {
      return { border: '0', background: 'transparent', color: 'var(--dsw-alias-fg-tertiary,#6f7c99)',
        cursor: 'pointer', fontSize: '12px', padding: '2px 6px', borderRadius: '4px', lineHeight: 1 }
    }
    ...
    function renderUserActions(text) {
      ...
      return React.createElement('div', { style: { display: 'flex', gap: '2px', marginTop: '2px' } },
        React.createElement('button', { type: 'button', title: '复制', onClick: function () { copyUserText(text) }, style: userActionStyle() }, '复制'),
        React.createElement('button', { type: 'button', title: '编辑', onClick: function () { editUserText(text) }, style: userActionStyle() }, '编辑'),
      )
    }
    ...
    function UserMessageView(props) {
      try {
        ...
        var actions = renderUserActions(text)
        if (parsed === null) {
