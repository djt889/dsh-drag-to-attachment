  ...
  factory: (require) => {
    ...
    function resolveFilePath(file, queued, ctx, failures) {
      return withTimeout(locateFileProgressive(file, ctx.workspaces, currentWorkspacePath(ctx), function (phase) {
        ...
        if (result.status === 'found') { queued.path = result.path; queued.phase = 'done'; stopRingProgress(queued); renderChips(); syncPlaceholder(); return }
        if (result.status === 'choose') {
          return choosePath(file.name, result.candidates).then(function (selected) {
            if (selected !== undefined) { queued.path = selected; queued.phase = 'done'; stopRingProgress(queued); renderChips(); syncPlaceholder() }
