  ...
  factory: (require) => {
    ...
    function handlePathDrop(ctx, dataTransfer, toast) {
      ...
      dropped.directories.forEach(function (directory) {
        entries.push({ name: directory.name, locate: function () { return locateDroppedDirectoryTimed(directory, ctx.workspaces, currentWorkspacePath(ctx), 20000) } })
      })
      dropped.files.forEach(function (file) {
        entries.push({ name: file.name, locate: function () { return locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 20000) } })
      })
      var found = []
      var failures = []
      var chain = Promise.resolve()
    ...
    function handlePathPaste(ctx, clipboardData, toast) {
      ...
      Promise.all(promises).then(function () {
        ...
        files.forEach(function (file) {
          chain = chain.then(function () { return locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 20000) }).then(function (result) {
            if (result.status === 'found') found.push(result.path)
            else if (result.status === 'choose') {
        ...
        dirs.forEach(function (dir) {
          chain = chain.then(function () { return locateDroppedDirectoryTimed(dir, ctx.workspaces, currentWorkspacePath(ctx), 20000) }).then(function (result) {
            if (result.status === 'found') found.push(result.path)
            else if (result.status === 'choose') {
    ...
    function resolveFilePath(file, queued, ctx, failures) {
      return locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 20000)
    ...
    function resolveDirPath(dir, queued, ctx, failures) {
      return locateDroppedDirectoryTimed(dir, ctx.workspaces, currentWorkspacePath(ctx), 20000)
        .then(function (result) {
          if (result.status === 'found') { queued.path = result.path; renderChips(); syncPlaceholder(); return }
          if (result.status === 'choose') {
            return choosePath(dir.name, result.candidates).then(function (selected) {
        ...
        if (hasImages && workspaces !== undefined) {
          ...
          if (attachments.length === imageIds.length) {
            ...
            for (var i = 0; i < attachments.length; i++) {
              ...
              if (!file) continue
              try {
                var result = await locateDroppedFileTimed(file, workspaces, cwd, 20000)
