 ...
 *    composer: images → native thumbnail cards; every other file → square
 *    chips in the attachment rail; folders → recursive upload shown as a
 *    folder chip. On send the conversation.sendSession prototype is
 *    wrapped: draft images + queued files/folders are uploaded into the
 *    workspace `.drops/` dir and the message sent to the model is the
 *    plain-text path list (+ your text) — a text-only model agent can read
 *    them via dsh-vision-toolkit / document tools, and the native image
 *    preflight (`attachment-error`) is bypassed because no image block
 *    reaches the wire.
 *
 *  PATH mode: drop or paste files anywhere and their REAL filesystem paths
 *    are located (Workspace-first, then registered workspaces, common
 *    dirs, OS index, bounded recursive search) and inserted into the draft
 *    as plain paths — nothing is uploaded or copied (ported from
 *    bill9109/dsh-drag-and-drop).
 *
 * All file types are accepted. Upload failures are never silent: a toast
 * explains before falling back to the native send path.
 */
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var IMPORT_ROUTE = '/_dsh/drag-to-attachment/import'
    ...
    /** Upload one file to the host and resolve { path, folderPath }. */
    function upload(file, workspace, relPath) {
    ...
    // --- FileSystemEntry tree reading (attachment mode folders) ---
    ...
    /**
     * Upload one folder: returns { folderPath, failed }. folderPath is the
     * workspace folder root or null (empty/unreadable); failed lists member
     * files that could not be uploaded — one failure never aborts the rest.
     */
    function uploadFolder(entry, name, workspace, toast) {
      var files = []
      return readEntryTree(entry, '', files, 0).then(function () {
        if (files.length === 0) return { folderPath: null, failed: [] }
        if (files.length >= MAX_DIR_FILES) {
          toast.showError('文件夹文件过多，仅上传前 ' + MAX_DIR_FILES + ' 个')
        }
        var chain = Promise.resolve()
        var folderPath = null
        var failed = []
        files.forEach(function (item) {
          chain = chain.then(function () { return upload(item.file, workspace, item.relPath) }).then(function (value) {
            if (folderPath === null && value && value.folderPath) folderPath = value.folderPath
          }).catch(function (error) {
            failed.push(item.file.name)
            console.error('[drag-to-attachment] folder member upload failed:', item.file.name, error)
          })
        })
        return chain.then(function () { return { folderPath: folderPath, failed: failed } })
      })
    }
    ...
    /** Turn top-level plain files (+ images when native rail unavailable) into chips. */
    function uploadFilesAsChips(files, workspace, toast) {
      var chain = Promise.resolve()
      files.forEach(function (item) {
        chain = chain.then(function () { return upload(item.file, workspace, item.relPath) }).then(function (value) {
          addQueued({ path: value.path, name: item.file.name })
        }).catch(function (error) {
          console.error('[drag-to-attachment] upload failed:', error)
          toast.showError('附件处理失败: ' + (error && error.message ? error.message : String(error)))
        })
      })
      return chain
    }

    /** Native draft-image cards for top-level images; fall back to chips. */
    function attachTopLevelImages(images, ctx, workspace, toast) {
      var conversation = ctx.get('conversation')
      var input = currentInput(ctx)
      if (conversation === undefined || input === undefined) {
        return uploadFilesAsChips(images.map(function (file) { return { file: file, relPath: undefined } }), workspace, toast)
      }
      try {
        var created = conversation.createDraftImages(images)
        if (input.addImages(created.map(function (a) { return a.id }))) return Promise.resolve()
        conversation.releaseDraftImages(created)
      } catch (error) { /* unsupported image type → chips */ }
      return uploadFilesAsChips(images.map(function (file) { return { file: file, relPath: undefined } }), workspace, toast)
    }

    function handleAttachmentDrop(ctx, dataTransfer, toast) {
      var conversation = ctx.get('conversation')
      if (conversation === undefined) { toast.showError('conversation 服务不可用'); return }
      collectDrop(dataTransfer).then(function (dropped) {
        var tasks = []
        var workspace = currentWorkspace(ctx.sessions)
        if (dropped.images.length > 0) {
          tasks.push(attachTopLevelImages(dropped.images, ctx, workspace, toast))
        }
        if (dropped.files.length > 0) {
          tasks.push(uploadFilesAsChips(dropped.files, workspace, toast))
        }
        dropped.dirs.forEach(function (dir) {
          tasks.push(uploadFolder(dir.entry, dir.name, workspace, toast).then(function (result) {
            if (result && result.folderPath !== null) {
              addQueued({ path: result.folderPath, name: dir.name, isDir: true })
              if (result.failed.length > 0) {
                toast.showError('文件夹 ' + dir.name + '：' + result.failed.length + ' 个文件上传失败（' +
                  result.failed.slice(0, 5).join('、') + (result.failed.length > 5 ? '…' : '') + '）')
              }
            } else {
              toast.showError('文件夹为空或无法读取：' + dir.name)
            }
          }).catch(function (error) {
            console.error('[drag-to-attachment] folder upload failed:', error)
            toast.showError('文件夹处理失败: ' + (error && error.message ? error.message : String(error)))
          }))
        })
    ...
    function handleAttachmentPaste(ctx, clipboardData, toast) {
      ...
      Promise.all(promises).then(function () {
        var tasks = []
        var workspace = currentWorkspace(ctx.sessions)
        if (images.length > 0) tasks.push(attachTopLevelImages(images, ctx, workspace, toast))
        if (files.length > 0) tasks.push(uploadFilesAsChips(files, workspace, toast))
        dirs.forEach(function (dir) {
          tasks.push(uploadFolder(dir.entry, dir.name, workspace, toast).then(function (result) {
            if (result && result.folderPath !== null) {
              addQueued({ path: result.folderPath, name: dir.name, isDir: true })
    ...
    // --- send-time path conversion (attachment mode) ---
    function patchSendSession(conversation, sessions) {
      ...
      proto.sendSession = async function (session, text, imageIds, mode) {
        ...
        if (!hasImages && queued.length === 0) return original.call(this, session, text, imageIds, mode)
        var attachments = []
        if (hasImages) {
        ...
        var paths = queued.map(function (item) { return item.path })
        try {
          if (hasImages) {
          ...
          console.error('[drag-to-attachment] image upload failed, sending as attachment:', error)
          toast.showError('附件上传失败，已按原生附件发送: ' + reason)
        ...
        // Strip the invisible send-enabler placeholder from the user text.
        var cleanedText = String(text || '').replace(/\u200C/g, '')
        var body = cleanedText.trim().length > 0 ? lines + '\n' + cleanedText : lines
