...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    function locateAndQueue(ctx, images, files, dirs, toast) {
      var found = []
      var failures = []
      var chain = Promise.resolve()
      // Images and plain files are all located by name/size/fingerprint.
      images.concat(files.map(function (item) { return item.file })).forEach(function (file) {
        chain = chain.then(function () { return locateDroppedFile(file, ctx.workspaces, currentWorkspacePath(ctx)) }).then(function (result) {
    ...
    function handleAttachmentDrop(ctx, dataTransfer, toast) {
      collectDrop(dataTransfer).then(function (dropped) {
        locateAndQueue(ctx, dropped.images, dropped.files, dropped.dirs, toast)
      }).catch(function (error) {
    ...
    function handleAttachmentPaste(ctx, clipboardData, toast) {
      ...
      Promise.all(promises).then(function () {
        locateAndQueue(ctx, images, files, dirs, toast)
      }).catch(function (error) {
    ...
    function patchSendSession(conversation, sessions, workspaces) {
      ...
      proto.sendSession = async function (session, text, imageIds, mode) {
        ...
        // Native draft images (only when attached via DSH's own channel):
        // locate their real paths too — nothing is uploaded or copied.
