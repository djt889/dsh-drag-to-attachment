    ...
    function handlePathPaste(ctx, clipboardData, toast) {
      ...
      Promise.all(promises).then(function () {
        ...
        files.forEach(function (file) {
          chain = chain.then(function () { return locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 8000) }).then(function (result) {
            if (result.status === 'found') found.push(result.path)
            else if (result.status === 'choose') {
        ...
        dirs.forEach(function (dir) {
          chain = chain.then(function () { return locateDroppedDirectoryTimed(dir, ctx.workspaces, currentWorkspacePath(ctx), 8000) }).then(function (result) {
            if (result.status === 'found') found.push(result.path)
            else if (result.status === 'choose') {
