  ...
  factory: (require) => {
    ...
    function renderChips() {
      ...
      fileQueue.forEach(function (item) {
        ...
        // Pending (path not resolved yet): a live progress bar at the bottom.
        if (!item.path) {
          var prog = document.createElement('span')
          prog.setAttribute('aria-label', item.phase === 'tier-full' ? '全盘搜索中' : '定位中')
          prog.style.cssText = 'position:absolute;bottom:0;left:0;right:0;height:3px;background:rgba(92,108,213,.25);overflow:hidden;z-index:2'
          var fill = document.createElement('span')
          fill.className = 'dta-progress-fill' + (item.phase === 'tier-full' ? ' slow' : '')
          fill.style.cssText = 'position:absolute;top:0;left:0;height:100%;width:40%;background:var(--dsw-alias-state-business-primary,#3964fe)'
          prog.append(fill)
          chip.append(prog)
        }
    ...
    function showPathPendingBar() {
      ...
      pathPending.forEach(function (item) {
        ...
        // Live progress bar (faster while tier-fast, slower while tier-full).
        var progress = document.createElement('span')
        progress.style.cssText = 'width:56px;height:4px;border-radius:2px;background:rgba(92,108,213,.2);overflow:hidden;position:relative;flex:0 0 auto'
        var fill = document.createElement('span')
        fill.className = 'dta-progress-fill' + (item.phase === 'tier-full' ? ' slow' : '')
        fill.style.cssText = 'position:absolute;top:0;left:0;height:100%;width:40%;background:var(--dsw-alias-state-business-primary,#3964fe);border-radius:2px'
        progress.append(fill)
        var label = document.createElement('span')
        label.textContent = item.name + (item.phase === 'tier-full' ? '（全盘搜索中…）' : '（快速定位中…）')
        chip.append(progress, label)
        bar.append(chip)
      })
      var card = findCard(ta)
      if (!card) return
    ...
    function addPathPending(name) { pathPending.push({ name: name, phase: 'tier-fast' }); showPathPendingBar() }
    function setPathPendingPhase(name, phase) {
      for (var i = 0; i < pathPending.length; i++) {
        if (pathPending[i].name === name) { pathPending[i].phase = phase; break }
      }
      showPathPendingBar()
    }
    ...
    function resolveFilePath(file, queued, ctx, failures) {
      return withTimeout(locateFileProgressive(file, ctx.workspaces, currentWorkspacePath(ctx), function (phase) {
        ...
        if (result.status === 'found') { queued.path = result.path; queued.phase = 'done'; renderChips(); syncPlaceholder(); return }
        if (result.status === 'choose') {
          return choosePath(file.name, result.candidates).then(function (selected) {
            if (selected !== undefined) { queued.path = selected; queued.phase = 'done'; renderChips(); syncPlaceholder() }
        ...
        return saveFileToWorkspace(file).then(function (value) {
          queued.path = value.path; queued.phase = 'done'; renderChips(); syncPlaceholder()
    ...
    function resolveDirPath(dir, queued, ctx, failures) {
      return withTimeout(locateDirProgressive(dir, ctx.workspaces, currentWorkspacePath(ctx), function (phase) {
        ...
        return saveFolderToWorkspace(dir.entry, dir.name).then(function (folderPath) {
          if (folderPath) { queued.path = folderPath; queued.phase = 'done'; renderChips(); syncPlaceholder() }
      ...
      try {
        ...
        if (!document.getElementById(styleId)) {
          ...
          style.textContent =
            ...
            '.dta-progress-fill{animation:dta-slide 1.2s ease-in-out infinite}' +
            '.dta-progress-fill.slow{animation-duration:2.4s}' +
            '@keyframes dta-slide{0%{left:-40%}100%{left:100%}}'
