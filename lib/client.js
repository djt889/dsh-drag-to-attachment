...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * and folder, then queue it as a chip. Nothing is uploaded or copied —
     * the queued path IS the original filesystem path. Images keep a
     * thumbnail preview on their chip (local blob), but on send EVERY
     * attachment is delivered as its real path (a text-only model reads it
     * with dsh-vision-toolkit; no image attachment ever hits the wire, so
     * the "model does not support images" preflight cannot fire).
     */
    function locateAndQueue(ctx, files, dirs, toast) {
      var found = []
      var failures = []
    ...
    function UserMessageView(props) {
      ...
      var bubble = {
        display: 'inline-block', maxWidth: '72%', padding: '8px 12px', borderRadius: '12px',
        background: 'var(--dsw-alias-bg-elevated, #ffffff)',
        color: 'var(--dsw-alias-fg-primary, #0f172a)',
        border: '1px solid var(--dsw-alias-border-l2, rgb(15 23 42 / 10%))',
        wordBreak: 'break-word',
      }
      var row = { display: 'flex', justifyContent: 'flex-end', margin: '6px 0' }
      if (parsed === null) {
      ...
      var cards = parsed.attachments.map(function (item, i) { return renderAttachmentCard(item, i) })
      return React.createElement('div', { style: row },
        React.createElement('div', { style: bubble },
