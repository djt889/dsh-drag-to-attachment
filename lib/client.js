...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * Slash commands (`/goal`, `/plan`, …) are sent through
     * `commands.execute(sessionId, line)`, NOT through `sendSession` — so
     * attachments queued in the composer would never leave. Before a command
     * runs, flush the current session's attachments as a normal message
     * (the patched sendSession builds the [附件] lines), then run the command.
     */
    function patchCommandExecute(ctx, sessions, conversation) {
      try {
        var remote = ctx.get('remote')
        if (remote === undefined && ctx.remote !== undefined) remote = ctx.remote
        var cmds = remote && remote.commands
        if (!cmds || typeof cmds.execute !== 'function' || cmds[PATCH_MARK]) return
        cmds[PATCH_MARK] = true
        var orig = cmds.execute.bind(cmds)
        cmds.execute = async function (sessionId, line) {
          if (currentQueue.length > 0 && conversation !== undefined) {
            var attach = currentQueue.filter(function (q) { return q.path !== '' })
            if (attach.length > 0) {
              try {
                var state = sessions.list.getSnapshot()
                var session = state.byId && state.byId[sessionId]
                if (session !== undefined) {
                  // Patched sendSession picks up currentQueue and clears it.
                  await conversation.sendSession(session, '', [], 'queue')
                }
              } catch (error) {
                console.error('[drag-to-attachment] command attach flush failed:', error)
              }
            }
          }
          return orig(sessionId, line)
        }
      } catch (error) {
        console.error('[drag-to-attachment] command patch failed:', error)
      }
    }
    ...
    function apply(ctx) {
      ...
      if (conversation !== undefined && sessions !== undefined) {
        ...
        patchCommandExecute(ctx, sessions, conversation)
      }
