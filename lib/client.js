...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function resolveFilePath(file, queued, ctx, failures) {
      return locateDroppedFileTimed(file, ctx.workspaces, currentWorkspacePath(ctx), 3500)
        .then(function (result) {
          if (result.status === 'found') { queued.path = result.path; renderChips(); return }
          if (result.status === 'choose') {
            return choosePath(file.name, result.candidates).then(function (selected) {
              if (selected !== undefined) { queued.path = selected; renderChips() }
    ...
    function resolveDirPath(dir, queued, ctx, failures) {
      return locateDroppedDirectory(dir, ctx.workspaces, currentWorkspacePath(ctx))
        .then(function (result) {
          if (result.status === 'found') { queued.path = result.path; renderChips(); return }
          if (result.status === 'choose') {
            return choosePath(dir.name, result.candidates).then(function (selected) {
          ...
          if (result.status === 'error') dropQueued(queued, failures, '文件夹「' + dir.name + '」：' + result.message)
          else dropQueued(queued, failures, '文件夹「' + dir.name + '」未能定位原始路径（请将文件夹放入工作区或常用目录）')
        })
        .catch(function (error) {
