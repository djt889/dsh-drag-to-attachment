...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function removeQueued(item) {
      currentQueue = currentQueue.filter(function (q) { return q !== item })
      renderChips()
      syncPlaceholder()
    }
    function clearFiles() {
    ...
    function ensureChipsObserver() {
      ...
      // stale and the chips never re-render after switching back. Body-level
      // observation re-renders the active session's chips whenever the
      // composer is rebuilt, and re-arms the per-session queue on the switch.
